...
export function openMovieModal(title = "Movie trailer") {
  ...
  dialog.innerHTML = `
    <div class="modal-heading"><h2 id="modal-title"></h2><button class="modal-close" type="button" aria-label="Close dialog" autofocus>&times;</button></div>
    <div class="modal-body"><p class="status-message" role="status">Loading...</p></div>`;
...
export function showMovieDetails(dialog, movie) {
  ...
  details.innerHTML = '<img alt="" width="500" height="750"><div><p class="metadata"></p><p class="genres"></p><p class="detail-overview"></p></div>';
  ...
  details.querySelector(".metadata").textContent = [
    getReleaseYear(movie.release_date), `\u2605 ${formatRating(movie.vote_average)}`, movie.runtime ? `${movie.runtime} min` : null,
  ].filter(Boolean).join(" ? ");
  details.querySelector(".genres").textContent = (movie.genres || []).map(genre => genre.name).join(" / ");
  details.querySelector(".detail-overview").textContent = movie.overview || "No description is currently available.";
